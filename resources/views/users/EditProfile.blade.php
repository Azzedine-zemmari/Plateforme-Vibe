        ...
        <form class="space-y-8" action="{{url('/UpdateProfile/'.$user->id)}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="flex justify-center">
                    <div class="relative">
                        ...
                        <div id="svg" class=" flex items-center justify-center">
                           <img  class="w-32 h-32 bg-gray-200 rounded-full" src="{{asset('/storage/'.$user->image)}}" alt="">
                        </div>
                        @endif
            ...
            <div>
                ...
                <textarea id="bio" name="bio" rows="4" placeholder="Enter Bio"
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">{{$user->bio}}</textarea>
            </div>
