    ...
    <div class="min-h-screen flex flex-col items-center justify-center py-12 sm:px-6 lg:px-8">
        ...
        <div class="w-full max-w-4xl bg-gradient-to-r from-blue-400 to-purple-500 rounded-t-lg shadow-md overflow-hidden mb-6">
            <div class="flex items-center justify-between px-6 py-4">
                ...
                <a href="{{ url('/users') }}" class="inline-flex items-center px-4 py-2 bg-white text-gray-700 rounded-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
        ...
        <div class="w-full max-w-4xl bg-white rounded-b-lg shadow-md p-6 space-y-6">
            ...
            <div class="flex items-center justify-center relative -mt-6 mb-6">
                <div class="relative w-32 h-32 bg-gray-200 rounded-full border-4 border-white overflow-hidden">
                    ...
                    @else
                        <img class="w-full h-full object-cover" src="{{ asset('/storage/' . $user->image) }}" alt="{{ $user->name }}'s Avatar">
                    @endif
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="flex space-x-2">
                    <a href="{{ url('/EditProfile/' . $user->id) }}" class="px-4 py-2 bg-black text-white text-sm rounded-md">
                        Edit Profile
                    </a>
            ...
            <!-- Posts Section -->
            <div class="space-y-4">
                <h2 class="text-lg font-medium text-gray-900">Posts ({{ $posts->count() }})</h2>
                @if ($posts->isEmpty())
                    ...
                    <ul class="space-y-2">
                        @foreach ($posts as $post)
                            <li class="flex justify-between items-center bg-gray-50 p-3 rounded-md">
                                <span class="font-semibold">{{ $post->content }}</span>
                                <span class="text-sm text-gray-500">Likes: {{ $post->likes_count }}, Comments: {{ $post->comments_count }}</span>
                            </li>
                        @endforeach
                    </ul>
                @endif
            ...
            <!-- Likes Section -->
            <div class="space-y-4">
                <h2 class="text-lg font-medium text-gray-900">Likes ({{ $likes->count() }})</h2>
                @if ($likes->isEmpty())
                    ...
                    <ul class="space-y-2">
                        @foreach ($likes as $like)
                            <li class="bg-gray-50 p-3 rounded-md">
                                Liked post: "{{$like->post->content ?? 'Post not available' }}"
                            </li>
                        @endforeach
                    </ul>
                @endif
            ...
            <!-- Comments Section -->
            <div class="space-y-4">
                <h2 class="text-lg font-medium text-gray-900">Comments ({{ $comments->count() }})</h2>
                @if ($comments->isEmpty())
